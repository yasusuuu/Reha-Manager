...
const [patientRemoteRefreshApplying, setPatientRemoteRefreshApplying] = useState(false);
  ...
  const patientSkipNextDirtyRef = useRef(false);
  const today = new Date();
  ...
  function applyPatientManagerRemoteData(data) {
    ...
    patientSkipNextDirtyRef.current = true;
  ...
  async function applyPendingPatientRemoteData() {
    if (patientRemoteRefreshApplying) return;
...
続行しますか？`
    ...
    setPatientRemoteRefreshApplying(true);
    patientRemoteApplyingRef.current = true;

    try {
      // 通知を受けた時点の一時データではなく、
      // ボタンを押した瞬間のFirestore最新版を取得して反映する。
      const latestSnapshot = await getDoc(doc(db, "settings", "patientManager"));

      if (!latestSnapshot.exists()) {
        alert("最新の患者振り分けデータを取得できませんでした。");
        return;
      }

      const latestData = latestSnapshot.data();
      applyPatientManagerRemoteData(latestData);

      // Reactのstate反映が完了してから編集可能状態へ戻す。
      await new Promise((resolve) => setTimeout(resolve, 50));

      patientBaseRevisionRef.current = Number(latestData.revision || 0);
      patientSaveStatusRef.current = "saved";
      setPatientSaveStatus("saved");
      setPatientRemoteUpdatePending(false);
      setPatientRemoteUpdatedByName("");
      ...
      console.error("patientManager latest data apply failed", error);
      alert("最新データの反映に失敗しました。通信状況を確認して、もう一度お試しください。");
    } finally {
      patientRemoteApplyingRef.current = false;
      setPatientRemoteRefreshApplying(false);
    }
  }
...
async function savePatientManagerData() {
  if (!patientDataReady) {
    alert("患者振り分けデータの読み込みが完了していません。");
    return;
  }

  if (patientRemoteApplyingRef.current || patientRemoteRefreshApplying) {
    alert("最新データを反映中です。反映完了後に保存してください。");
    return;
  }

  if (patientSavingRef.current || patientSaving) return;
  ...
  patientSavingRef.current = true;
  setPatientSaving(true);
  patientSaveStatusRef.current = "saving";
  ...
  try {
    ...
    if (error?.code === "patient-manager/conflict") {
      ...
      patientSaveStatusRef.current = "dirty";
      setPatientSaveStatus("dirty");
    ...
    patientSavingRef.current = false;
...
useEffect(() => {
  ...
  // 他者の最新版をstateへ流し込んだ直後の再レンダーは、
  // ユーザー編集ではないためdirtyにしない。
  if (patientSkipNextDirtyRef.current) {
    patientSkipNextDirtyRef.current = false;
    patientSaveStatusRef.current = "saved";
    setPatientSaveStatus("saved");
    return;
  }
      ...
      {patientRemoteUpdatePending && (
        ...
        >
          ...
          <button
            ...
            disabled={patientRemoteRefreshApplying}
          >
            {patientRemoteRefreshApplying ? "反映中..." : "最新データを反映"}
          </button>
        </div>
      )}
